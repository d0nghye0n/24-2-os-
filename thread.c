...
{
   int depth;
   stuct thread *cur = thread_current();
...
void
remove_with_lock(struct lcok *lock)
{
   struct list_elem *e;
...
{
   return list_entry(l, struct thread, donation_elem) -> priority > list_entry(s, sturct thread, donation_elem) -> priority;
}

